...
public final class AntColonyOptimization {
  ...
  private static String TSP_FILE = "files/test.tsp";
  // greedy
  ...
  private final ExecutorCompletionService<WalkedWay> agentCompletionService = new ExecutorCompletionService<WalkedWay>(THREAD_POOL);
  ...
  private final double[][] readMatrixFromFile() throws IOException {
    ...
    while ((line = br.readLine()) != null) {
      ...
      if (readAhead) {
        ...
        records.add(new Record(Integer.parseInt(split[0].trim()),
        		Double.parseDouble(split[1].trim()), 
        		Double.parseDouble(split[2].trim())));
      }

      if (line.equals("NODE COORDINATES")) {
        readAhead = true;
      }
    }
    ...
    for (Record r : records) {
      ...
      for (Record h : records) {
    	  localMatrix[rIndex][hIndex] = distFrom(r.x, r.y, h.x, h.y);
  ...
  // Calculate distance in meters from given coordinates
  private final double distFrom(double lat1, double lng1, double lat2, double lng2) {
  ...
  static class Record {
    ...
    public Record(int i, double x, double y) {
      super();
      this.id = i;
      this.x = x;
      this.y = y;
  ...
  public static void main(String[] args) throws IOException, InterruptedException, ExecutionException {
    ...
    System.out.println("Result was: " + result +" meters.");
    
    PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter("files/results.txt",true)));
    out.println("Path: " + result + " (in " + (System.currentTimeMillis() - start) + ") -> ");
    out.close();
  }
